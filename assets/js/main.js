    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.creator) {
                ...
                if (creatorAvatar) {
                    creatorAvatar.src = data.creator.avatar_url;
                    creatorAvatar.alt = data.creator.display_name;
                }
                if (creatorName) {
                    creatorName.textContent = data.creator.display_name;
                }
            }
            ...
            if (testersGrid) {
                ...
                if (data.testers && data.testers.length > 0) {
                    data.testers.forEach(tester => {
                        const testerEl = document.createElement('div');
                        testerEl.className = 'team-member tester-member';
                        testerEl.innerHTML = `
                            <img src="${tester.avatar_url}" alt="${tester.display_name}" class="member-avatar">
                            <div class="member-info">
                                <span class="member-name">${tester.display_name}</span>
                                <span class="member-role">Tester</span>
                            </div>
                        `;
                        testersGrid.appendChild(testerEl);
                    });
                } else {
                    testersGrid.innerHTML = `
                        <div class="no-testers">
                            <p>No hay testers actualmente</p>
                        </div>
                    `;
                }
            }
            ...
            if (collaboratorsGrid) {
                ...
                if (data.collaborators && data.collaborators.length > 0) {
                    data.collaborators.forEach(collaborator => {
                        ...
                        collaboratorEl.className = 'team-member collaborator-member';
                        collaboratorEl.innerHTML = `
                            <img src="${collaborator.avatar_url}" alt="${collaborator.display_name}" class="member-avatar">
                            <div class="member-info">
                                <span class="member-name">${collaborator.display_name}</span>
                                <span class="member-role">Colaborador</span>
                            </div>
                        `;
                        collaboratorsGrid.appendChild(collaboratorEl);
                    });
                } else {
                    collaboratorsGrid.innerHTML = `
                        <div class="no-collaborators">
                            <p>No hay colaboradores actualmente</p>
                        </div>
                    `;
                }
            }
